...
PY
    ...
    case "$BUILD_DIR" in
      ...
      *) build_dir_abs="/workspace/$BUILD_DIR" ;;
    esac
    export BUILD_DIR="$build_dir_abs"
    ...
    if [ "$USE_UPSTREAM_CACHE" = "1" ]; then
      if [ ! -d "$BUILD_DIR/.git" ]; then
        ...
        if git clone --depth 1 --branch "$UPSTREAM_CACHE_REF" "$UPSTREAM_CACHE_REPO" "$BUILD_DIR"; then
          (cd "$BUILD_DIR" && git lfs pull)
        else
          echo "WARNING: upstream build cache unavailable; building dependencies from source"
          rm -rf "$BUILD_DIR"
          mkdir -p "$BUILD_DIR"
        fi
      else
      ...
      if find "$BUILD_DIR" -name "cache-*.tar.gz" -print -quit | grep -q .; then
        (cd "$BUILD_DIR" && python3 /workspace/nix/cache_dependencies.py unpack)
        python3 - <<PY
from pathlib import Path
...
PY
        ...
        echo "No upstream dependency cache archives found; build-all.py will build dependencies."
      fi
    else
      mkdir -p "$BUILD_DIR"
    fi
    ...
    python3 ./nix/build-all.py -v --diskcleanup "$PYTHON_BUILD_FLAG" IfcOpenShell-Python

    deps_dir="$BUILD_DIR/Linux/x86_64"
    module_root="$deps_dir/install/ifcopenshell/python-$PYTHON_VERSION"
    if [ ! -d "$module_root" ]; then
      module_root="$(find "$deps_dir/install/ifcopenshell" -maxdepth 1 -type d -name "python-3.11*" | sort | head -n 1)"
    fi
    if [ -z "${module_root:-}" ] || [ ! -d "$module_root" ]; then
      echo "ERROR: py311 ifcopenshell module output not found under $deps_dir/install/ifcopenshell"
      exit 1
    fi

    if [ -d "$module_root/ifcopenshell" ]; then
      module_pkg="$module_root/ifcopenshell"
    else
...
PY
    ...
    cp "$wrapper_so" "$WHEEL_BUILD_DIR/ifcopenshell/"
    wrapper_py="$(find "$module_pkg" -maxdepth 1 -name "ifcopenshell_wrapper.py" -print -quit)"
    if [ -n "$wrapper_py" ]; then
...
PY
    ...
    if [ -x "$verify_python" ]; then
      "$verify_python" -m venv "/workspace/$WHEEL_TEMP_DIR/verify-venv"
      "/workspace/$WHEEL_TEMP_DIR/verify-venv/bin/python" -m pip install --upgrade pip >/dev/null
      "/workspace/$WHEEL_TEMP_DIR/verify-venv/bin/python" -m pip install "$final_wheel" numpy >/dev/null
      "/workspace/$WHEEL_TEMP_DIR/verify-venv/bin/python" - <<PY
import ifcopenshell
import ifcopenshell.geom
print("verify import", ifcopenshell.version)
PY
    else
      echo "WARNING: built Python $PYTHON_VERSION executable not found; skipped wheel import verification"
    fi
