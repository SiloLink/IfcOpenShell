...
struct indexed_colour_mapping {
	std::vector<ifcopenshell::geometry::taxonomy::style::ptr> styles;
	std::vector<int> face_style_indices;
};

template <typename FaceSet>
indexed_colour_mapping map_indexed_colours(const FaceSet* inst, size_t face_count) {
	...
	auto colour_map = *colour_maps->begin();
	auto colour_list = colour_map->Colours();
	...
	auto palette = colour_list ? colour_list->ColourList() : std::vector<std::vector<double>>{};
	...
	for (size_t i = 0; i < colour_indices.size(); ++i) {
		result.face_style_indices[i] = colour_indices[i] - 1;
	}
